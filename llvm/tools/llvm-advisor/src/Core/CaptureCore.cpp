...
Error synthesizeArtifacts(CapabilityContext &Context, StringRef StoreRoot,
  ...
  Expected<SmallString<256>> ArtDirOrErr =
      artifactDir(StoreRoot, Context.Unit.ID);
  if (!ArtDirOrErr)
...
Expected<SnapshotRecord> CaptureCore::initializeSnapshot(StringRef SourceRoot,
                                                         StringRef BuildRoot) {
...
Expected<UnitRecord> CaptureCore::prepareUnit(const CompileCommand &Command,

...
static HTTPResult handleGetSummary(CoreClient &Client, StringRef SnapID) {
  ...
  for (const CapabilitySpec &Spec : Client.listCapabilities())
    if (shouldQuerySummaryCapability(Spec.ID) &&
        isSummarySafeCapability(Spec.ID))
    ...
    for (const json::Value &UnitValue : *Query) {
      ...
      for (const json::Value &ResultValue : *Results) {
        ...
        if (*Capability == "llvm.ir.summary") {
          Instructions +=
              ValueObj->getInteger("instructions")
                  .value_or(
                      ValueObj->getInteger("instruction_count").value_or(0));
          Functions +=
              ValueObj->getInteger("functions")
          ...
          Remarks += ValueObj->getInteger("count").value_or(
              ValueObj->getInteger("remark_count").value_or(0));
        }
      }
    }
  } else {
        ...
        if (Path == "/")
          ...
          if (Segs.size() == 4)
            ...
            std::string BL = BIt != QueryParams.end()
                                 ...
                                 : "";
            Res = handleGetInsights(Client, ResolvedSnap, Segs[5],
                                    /*UnitID=*/"", BL);
          ...
          Res = handleGetBlob(Client, Segs[3]);
                 ...
                 Segs[4] == "correlate")
          Res =
              handleGetCorrelate(Client, resolveSnapshotHTTP(Client, Segs[3]));
        else if (IsAPI && Segs.size() == 6 && Segs[2] == "query" &&
                 Segs[3] == "unit")
          Res = handleGetQueryUnit(Client, urlDecode(Segs[4]), Segs[5]);
        else if (IsAPI && Segs.size() == 6 && Segs[2] == "query" &&
                 Segs[3] == "snapshot")
          Res = handleGetQuerySnapshot(
              Client, resolveSnapshotHTTP(Client, urlDecode(Segs[4])), Segs[5]);
        else if (IsAPI && Segs.size() == 5 && Segs[2] == "compare")
          Res = handleGetCompare(
              Client, resolveSnapshotHTTP(Client, urlDecode(Segs[3])),
              resolveSnapshotHTTP(Client, urlDecode(Segs[4])));
        else if (IsAPI && Segs.size() == 7 && Segs[2] == "compare" &&
                 Segs[5] == "capability")
          Res = handleGetCompareCapability(
              Client, resolveSnapshotHTTP(Client, urlDecode(Segs[3])),
              resolveSnapshotHTTP(Client, urlDecode(Segs[4])),
              urlDecode(Segs[6]));
        else if (!IsAPI)

...
static const UnitRecord *
matchUnit(const UnitRecord &CandUnit,
          const StringMap<const UnitRecord *> &BaseByID,
          const StringMap<const UnitRecord *> &BaseByPath, StringSet<> &Seen) {
  if (auto It = BaseByID.find(CandUnit.ID); It != BaseByID.end()) {
    Seen.insert(CandUnit.ID);
    return It->second;
  }
  if (auto It = BaseByPath.find(CandUnit.SourcePath); It != BaseByPath.end()) {

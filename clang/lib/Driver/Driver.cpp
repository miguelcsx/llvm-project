...
Compilation *Driver::BuildCompilation(ArrayRef<const char *> ArgList) {
  ...
  if (!Triple.isWasm() && !Triple.isSPIRV()) {
...
Action *Driver::BuildOffloadingActions(Compilation &C,
  ...
  for (Action::OffloadKind Kind : OffloadKinds) {
    ...
    for (phases::ID Phase : PL) {
      ...
      // Keep Apple Metal OpenMP images as LLVM bitcode until the final link so
      // the linker wrapper can derive structured kernel metadata before the
      // image is lowered to a .metallib.
      if (Kind == Action::OFK_OpenMP && Phase == phases::Assemble &&
          llvm::all_of(TCAndArchs, [](const auto &TCAndArch) {
            const llvm::Triple &TT = TCAndArch.first->getTriple();
            return TT.isSPIRV() && TT.isOSDarwin();
          }))
        continue;
...
Action *Driver::ConstructPhaseAction(
  ...
  case phases::Backend: {
    ...
    bool EmitBitcodeForMetalOpenMPSPIRV =
        TargetDeviceOffloadKind == Action::OFK_OpenMP && OffloadingToolChain &&
        OffloadingToolChain->getTriple().isSPIRV() &&
        OffloadingToolChain->getTriple().isOSDarwin();

    if (Args.hasArg(options::OPT_emit_llvm) ||
        EmitBitcodeForNonOffloadAMDSPIRV || EmitBitcodeForMetalOpenMPSPIRV ||

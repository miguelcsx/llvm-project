...
void tools::addOpenMPDeviceRTL(const Driver &D,
  ...
  if (Triple.isSPIRV()) {
    SmallString<128> GenericArchPath(D.Dir);
    llvm::sys::path::append(GenericArchPath, "..", "lib", Triple.getArchName());
    LibraryPaths.emplace_back(GenericArchPath);

    SmallString<128> GenericTriplePath(D.Dir);
    llvm::sys::path::append(GenericTriplePath, "..", "lib",
                            Triple.getArch() == llvm::Triple::spirv32
                                ? "spirv32-unknown-unknown"
                                : "spirv64-unknown-unknown");
    LibraryPaths.emplace_back(GenericTriplePath);
  }
